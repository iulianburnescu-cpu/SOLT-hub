...
const inpS = {
  ...
  fontSize: 14, fontFamily: 'inherit', color: BK, background: WH, outline: 'none', boxSizing: 'border-box',
};

function Lbl({ children }) {
  return <div style={{ fontSize: 12, fontWeight: 600, color: GR, marginBottom: 4, textTransform: 'uppercase', letterSpacing: .5 }}>{children}</div>;
}

function Tag({ children, yellow }) {
  return <span style={{ fontSize: 12, padding: '2px 8px', borderRadius: 99, background: yellow ? '#FFF9CC' : BG, border: `1px solid ${BR}`, color: yellow ? '#7A6000' : GR, whiteSpace: 'nowrap' }}>{children}</span>;
}
...
function Btn({ children, onClick, ghost, disabled }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      cursor: disabled ? 'not-allowed' : 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'inherit',
    }}>{children}</button>
  );
}

function StepRow({ step, idx, open, onToggle, onUpdate }) {
    ...
    <div style={{ border: `1px solid ${open ? BK : BR}`, borderRadius: 10, overflow: 'hidden', marginBottom: 5, background: WH, transition: 'border-color .15s' }}>
      <div onClick={onToggle} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 13px', cursor: 'pointer', userSelect: 'none' }}>
        <div style={{ width: 22, height: 22, borderRadius: 6, background: open ? Y : BG, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'background .15s' }}>
          <span style={{ fontSize: 12, fontWeight: 700, color: open ? WH : GR }}>{idx + 1}</span>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 600, fontSize: 15, lineHeight: 1.1 }}>{step.name}</div>
          {step.sub && <div style={{ fontSize: 13, color: GR, marginTop: 1 }}>{step.sub}</div>}
        </div>
        <div style={{ width: 7, height: 7, borderRadius: '50%', background: st.dot, flexShrink: 0 }} />
        <span style={{ fontSize: 12, padding: '2px 8px', borderRadius: 99, background: st.bg, color: st.c, border: `1px solid ${BR}`, whiteSpace: 'nowrap', flexShrink: 0 }}>{st.l}</span>
            ...
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 4 }}>
              {Object.entries(STATUS).map(([k, v]) => (
                <button key={k} onClick={() => onUpdate({ status: k })} style={{
                  padding: '3px 9px', borderRadius: 99, border: `1px solid ${BR}`, cursor: 'pointer', fontSize: 12,
            ...
            {step._editing ? (
              <div>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6, padding: '6px 8px', background: '#f9f5ff', border: `1px solid ${BR}`, borderTop: `1px solid #e8d8fc`, borderBottomLeftRadius: 8, borderBottomRightRadius: 8 }}>
                  <button onClick={() => onUpdate({ _editing: false })} style={{ padding: '4px 12px', borderRadius: 99, border: `1px solid ${BR}`, background: WH, color: GR, cursor: 'pointer', fontSize: 13, fontFamily: 'inherit' }}>Anulează</button>
                  <button onClick={() => onUpdate({ _editing: false })} style={{ padding: '4px 12px', borderRadius: 99, border: 'none', background: Y, color: WH, cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'inherit' }}>Salvează</button>
                </div>
              </div>
            ) : (
              <div onClick={() => onUpdate({ _editing: true })} style={{
                fontSize: 15, lineHeight: 1.8, color: step.notes ? BK : GR,
    ...
    <div>
      <div style={{ marginBottom: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
          <span style={{ fontSize: 13, color: GR }}>{done}/{steps.length} categorii finalizate</span>
          <span style={{ fontSize: 13, color: GR }}>{inProgress} în lucru · <span style={{ color: blocked > 0 ? '#C0392B' : GR }}>{blocked} blocate</span></span>
        </div>
      ...
      {adding ? (
        <div style={{ border: `1px solid ${BK}`, borderRadius: 10, padding: 13, marginTop: 6, background: '#FAFAFA' }}>
          ...
          <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
            <button onClick={() => { setAdding(false); setNewName(''); setNewSub(''); }} style={{ padding: '5px 14px', borderRadius: 99, border: `1px solid ${BR}`, background: WH, color: GR, cursor: 'pointer', fontSize: 13, fontFamily: 'inherit' }}>Anulează</button>
            <button onClick={handleAdd} disabled={!newName.trim()} style={{ padding: '5px 14px', borderRadius: 99, border: 'none', background: newName.trim() ? Y : '#ccc', color: WH, cursor: newName.trim() ? 'pointer' : 'not-allowed', fontSize: 13, fontWeight: 600, fontFamily: 'inherit' }}>Adaugă</button>
          </div>
        </div>
      ) : (
        <button onClick={() => setAdding(true)} style={{ width: '100%', marginTop: 6, padding: '8px 0', borderRadius: 10, border: `1px dashed #c4a8f0`, background: '#f9f5ff', color: Y, cursor: 'pointer', fontSize: 14, fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
...
function TodoItem({ todo, steps, onToggle, onDelete }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 9, padding: '9px 13px', border: `1px solid ${BR}`, borderRadius: 10, background: todo.done ? BG : WH, marginBottom: 5 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 15, color: todo.done ? GR : BK, textDecoration: todo.done ? 'line-through' : 'none', lineHeight: 1.3 }}>{todo.title}</div>
        <div style={{ display: 'flex', gap: 5, marginTop: 4, flexWrap: 'wrap', alignItems: 'center' }}>
          {todo.assignee && <Tag>{todo.assignee}</Tag>}
          {step && <Tag yellow>{step.name}</Tag>}
          {todo.dueDate && <span style={{ fontSize: 12, color: over ? '#C0392B' : GR }}>{over ? '⚠ ' : ''}{fmtDate(todo.dueDate)}</span>}
        </div>
      </div>
    ...
    <div>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {[['open', `Deschise${openCount > 0 ? ` (${openCount})` : ''}`], ['all', 'Toate'], ['done', 'Completate']].map(([f, l]) => (
            <button key={f} onClick={() => setFilter(f)} style={{
              padding: '4px 12px', borderRadius: 99, border: 'none', cursor: 'pointer', fontSize: 13,
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
        <button onClick={() => setCatFilter('')} style={{
          ...
          cursor: 'pointer', fontSize: 13, fontWeight: catFilter === '' ? 600 : 400, fontFamily: 'inherit',
        ...
        {steps.filter(s => todos.some(t => t.stepId === s.id)).map(s => (
          <button key={s.id} onClick={() => setCatFilter(catFilter === s.id ? '' : s.id)} style={{
            ...
            cursor: 'pointer', fontSize: 13, fontWeight: catFilter === s.id ? 600 : 400, fontFamily: 'inherit',
          }}>{s.name}</button>
        ))}
      </div>
      ...
      {shown.length === 0 && !showForm && (
        <div style={{ textAlign: 'center', color: GR, fontSize: 15, padding: '48px 0' }}>
    ...
    <div style={{ border: `1px solid ${BR}`, borderRadius: 10, background: WH, overflow: 'hidden', marginBottom: 6 }}>
      <div onClick={() => setOpen(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 13px', cursor: 'pointer' }}>
        <div style={{ width: 38, textAlign: 'center', flexShrink: 0 }}>
          ...
          <div style={{ fontSize: 11, color: GR, textTransform: 'uppercase' }}>{months[d.getMonth()]}</div>
        </div>
        <div style={{ width: 1, height: 32, background: BR, flexShrink: 0 }} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 600, fontSize: 15, lineHeight: 1.1 }}>{minute.title}</div>
          {minute.participants?.length > 0 && <div style={{ fontSize: 13, color: GR, marginTop: 2 }}>{minute.participants.join(', ')}</div>}
        </div>
      ...
      {open && (
        <div style={{ borderTop: `1px solid ${BR}`, padding: '12px 14px', background: '#FAFAFA' }}>
          {minute.notes && <div style={{ marginBottom: 10 }}><Lbl>Note</Lbl><div style={{ fontSize: 14, lineHeight: 1.7, whiteSpace: 'pre-wrap', color: BK }}>{minute.notes}</div></div>}
          {decisions.length > 0 && (
            <div style={{ marginBottom: 10 }}>
              ...
              {decisions.map((dec, i) => (
                <div key={i} style={{ display: 'flex', gap: 7, alignItems: 'flex-start', marginBottom: 4 }}>
                  ...
                  <div style={{ fontSize: 14, lineHeight: 1.5, color: BK }}>{dec}</div>
                </div>
              ))}
            </div>
          )}
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              {actions.map((act, i) => (
                <div key={i} style={{ display: 'flex', gap: 7, alignItems: 'flex-start', marginBottom: 4 }}>
                  ...
                  <div style={{ fontSize: 14, lineHeight: 1.5, color: BK }}>{act}</div>
                </div>
              ))}
            </div>
          )}
          <button onClick={onDelete} style={{ fontSize: 13, color: '#C0392B', border: 'none', background: 'none', cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}>Șterge minuta</button>
        </div>
      )}
    </div>
  );
        ...
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginTop: 4 }}>
          {people.map(p => (
            <button key={p} onClick={() => tog('participants', p)} style={{
              padding: '3px 10px', borderRadius: 99, border: `1px solid ${BR}`, cursor: 'pointer', fontSize: 13,
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <span style={{ fontSize: 14, color: GR }}>{minutes.length} {minutes.length === 1 ? 'ședință' : 'ședințe'} înregistrate</span>
      ...
      {minutes.length === 0 && !showForm && (
        <div style={{ textAlign: 'center', color: GR, fontSize: 15, padding: '48px 0' }}>Nicio ședință înregistrată.</div>
      )}
  ...
  return (
    <div style={{ position: 'absolute', top: 48, right: 0, width: 210, background: WH, border: `1px solid ${BR}`, borderRadius: 10, boxShadow: '0 4px 20px rgba(0,0,0,.1)', zIndex: 200, padding: 12 }}>
      ...
      {people.map((p, i) => (
        <div key={p} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 0', borderBottom: `1px solid ${BG}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            <div style={{ width: 22, height: 22, borderRadius: '50%', background: BG, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 600, color: GR }}>{p[0]}</div>
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
        ...
        <button onClick={add} style={{ padding: '4px 10px', borderRadius: 6, border: 'none', background: BK, color: WH, cursor: 'pointer', fontSize: 14, fontFamily: 'inherit' }}>+</button>
      </div>
    </div>
  );
}
    ...
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", background: WH, color: BK, minHeight: '100vh', position: 'relative' }}>
      <div style={{ position: 'sticky', top: 0, zIndex: 100, background: '#5e35b1', borderBottom: `1px solid #4a2899`, padding: '0 16px', display: 'flex', alignItems: 'center', gap: 10, height: 46 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 15, letterSpacing: '-.3px', color: 'white' }}>SOLT Hub</span>
        </div>
        <div style={{ width: 1, height: 18, background: 'rgba(255,255,255,0.2)' }} />
        <div style={{ display: 'flex', gap: 2 }}>
          {[['steps', 'Categorii'], ['todos', openCount > 0 ? `To-dos (${openCount})` : 'To-dos'], ['minutes', 'Minute']].map(([id, l]) => (
            <button key={id} onClick={() => setTab(id)} style={{
              padding: '3px 11px', borderRadius: 99, border: 'none', cursor: 'pointer', fontSize: 13,
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8, position: 'relative' }}>
          {loaded && (
            <span style={{ fontSize: 12, color: saved ? 'rgba(255,255,255,0.7)' : 'rgba(255,255,255,0.4)', display: 'flex', alignItems: 'center', gap: 3 }}>
      ...
      <div style={{ padding: 16, maxWidth: 740, margin: '0 auto' }}>
        {error && <div style={{ marginBottom: 12, padding: '10px 14px', background: '#FEE8E8', border: '1px solid #F5C6C6', borderRadius: 8, fontSize: 14, color: '#C0392B' }}>⚠ {error}</div>}
